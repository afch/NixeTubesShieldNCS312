...
//Format                _X.XX__    
//NIXIE CLOCK SHIELD NCS312 by GRA & AFCH (fominalec@gmail.com)
//1.021 17.10.2016
//Added: self test
//1.02 17.10.2016
...
void doIndication()
{
  ...
  tmpVar64=(SymbolArray[digits%10]|doEditBlink(0))<<6>>4;
  Var64|=tmpVar64;
  
  Var64=(Var64>>2);

  boolean delme=false;
  if (LD) Var64&=~LowerDotsMask;
    else  Var64|=LowerDotsMask;
  
  if (UD) Var64&=~UpperDotsMask; 
    else  Var64|=UpperDotsMask;
...
//  if (Uinput<10) testStringArray[10]="000"+String(int(Uinput*100)); else testStringArray[10]="00"+String(int(Uinput*100));
  ...
  for (byte i=0; i<11; i++)
  {
   if ((millis()-startOfTest)>dlay) 
   {
    ...
    tmpVar64=(SymbolArray[digits%10])<<6>>4;
    Var64|=tmpVar64;
